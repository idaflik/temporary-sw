...
# write_csv(country_names, "output/unique_country_names.csv")
valid_country_names <- read_csv("output/valid_country_names.csv")%>%
  rename(country_name_clean = country)

replace_names <- c(
  "Bosna and Herzegovina" = "Bosnia and Herzegovina",
  "Bosnia&Herzeg ovina"  = "Bosnia and Herzegovina",
  ...
  "Former Yugoslav Republic of Macedonia" = "Macedonia",
  "Republic of N.Macedonia" = "Macedonia",
...
data_allyears_clean <- data_allyears %>%
  mutate(across(c(country, beneficiary), ~if_else(. %in% names(replace_names), str_replace_all(., replace_names), .)))%>%
